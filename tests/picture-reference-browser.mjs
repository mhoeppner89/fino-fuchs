...
for (const [engine, launcher] of Object.entries({ chromium, webkit })) {
  ...
  try {
    ...
    await page.evaluate(async () => {
      const { buildSession, seededRandom } = await import('./js/curriculum.js?v=1.3.49');
    ...
    const tasks = await page.evaluate(async () => (await import('./js/curriculum.js?v=1.3.49')).EXERCISE_BANKS.shapes.map(t => ({ id: t.id, title: t.title })));
    for (const task of tasks) {
      const pass = await page.evaluate(async (id) => {
        const { EXERCISE_BANKS, adaptTaskToViewport } = await import('./js/curriculum.js?v=1.3.49');
        const { passesDrawingCriteria } = await import('./js/drawing.js?v=1.3.49');
