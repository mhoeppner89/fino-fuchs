...
for (const [engineName, engine] of engines) {
  ...
  try {
    ...
    await page.evaluate(async () => {
      const { EXERCISE_BANKS, adaptTaskToViewport } = await import('../js/curriculum.js?v=1.3.41');
      const board = window.__fuchsschrift.board;
    ...
    assert.equal(await page.evaluate(async () => Boolean(await caches.match('./js/stroke-validation.js?v=1.3.41'))), true);
